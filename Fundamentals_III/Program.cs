...
SquareValues(TestIntList3).ForEach(x=>Console.WriteLine(x));
...
NonNegatives(TestIntArray).ToList().ForEach(x=>Console.WriteLine(x));
...
List<string> tL1 = new List<string>{"Julie", "Harold", "James", "Monica"};
List<int> tL2 = new List<int>{6,12,7,10};
foreach(KeyValuePair<string, int> kvp in GenerateDictionary(tL1, tL2))
{
